...
export default function TitleAndQuestions({ data }) {
  ...
  return (
    <>
      <h2 className="faq-heading text-[clamp(14px,4vw,16px)] leading-[clamp(22px,4vw,24px)] mb-5 uppercase lg:w-1/2">
        ({data.heading})
      </h2>
        ...
        {data.items.map((item, index) => (
          ...
          >
            <div className="flex items-center gap-1 justify-between md:gap-5">
              <h3 className="faq-title text-[clamp(14px,4.5vw,20px)]  leading-[clamp(24px,6vw,28px)] lg:text-[clamp(16px,5vw,26px)] lg:leading-[clamp(26px,6vw,36px)] pl-2.5">
                {item.question}
              </h3>
